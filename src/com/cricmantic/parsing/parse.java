...
public class parse {
	...
	parse()
	{
		model.read("C:\\Users\\Hamza\\workspace\\pro-master\\pro-master\\src\\Archive\\cric.rdf");
	}
	...
	public void parseCommentary(String data,int inn) throws IOException
	{
		...
		{
			String content = null;
			String fileName="C:\\Users\\Hamza\\workspace\\pro-master\\pro-master\\src\\Archive\\balls.txt";
		...
		for(i=0;i<data.length()-3;i++)
		{
			...
			if(Character.isDigit(data.charAt(i))&& Character.isDigit(data.charAt(i+1))
					&& data.charAt(i+2)=='.'&& Character.getNumericValue(data.charAt(i+3))>0&&Character.getNumericValue(data.charAt(i+3))<=6 && Character.getNumericValue(data.charAt(i+4))==-1&& !special.matcher(Character.toString(data.charAt(i+3))).find())
			{
			...
			if(Character.isDigit(data.charAt(i)) &&data.charAt(i+1)=='.' && 
					Character.getNumericValue(data.charAt(i+2))>0&&Character.getNumericValue(data.charAt(i+2))<=6 && Character.getNumericValue(data.charAt(i+3))==-1 && !special.matcher(Character.toString(data.charAt(i+3))).find())
			{
		...
		if(inn==2)
		{
			...
			content = br.readLine(); 
			balls=Integer.parseInt(content);
			fileName = "C:\\Users\\Hamza\\workspace\\pro-master\\pro-master\\src\\Archive\\cricmantic"+Integer.toString(balls)+".owl";
